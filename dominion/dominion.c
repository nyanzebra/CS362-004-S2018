...
int adventurerEffect(struct gameState* state, int drawntreasure, int currentPlayer, int z, int cardDrawn, int temphand[]) {
...
// bug will make all return -1 be 4,294,967,295...
unsigned int mineEffect(int choice1, int choice2, struct gameState *state, int handPos, int currentPlayer, int* j) {
    int i = 0;
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card )
    {
    case adventurer:
      return adventurerEffect(state, drawntreasure, currentPlayer, z, cardDrawn, temphand);

    case council_room:
      return council_roomEffect(state, handPos, currentPlayer);
    ...
    case mine:
      return mineEffect(choice1, choice2, state, handPos, currentPlayer, &j);
